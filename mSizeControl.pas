...
uses
  Windows, Messages, SysUtils, Classes, Controls, ExtCtrls, ComCtrls, CommCtrl,
...
type
  ...
  TBtnPos = (bpLeft, bpTopLeft, bpTop, bpTopRight, bpRight, bpBottomRight, bpBottom, bpBottomLeft);
  TBtnPosSet = set of TBtnPos;

  TSCState = (scsReady, scsMoving, scsSizing);

  TStartEndEvent = procedure (Sender: TObject;
    State: TSCState) of object;
  TDuringEvent = procedure (Sender: TObject;
    dx, dy: integer; State: TSCState) of object;
  ...
  public
    constructor Create(TargetObj: TTargetObj; BtnPos: TBtnPos); {$IFNDEF VER100} reintroduce; {$ENDIF}
  end;
  ...
  public
    constructor Create(aSizeCtrl: TSizeCtrl; aControl: TControl); {$IFNDEF VER100} reintroduce; {$ENDIF}
  ...
  public
    constructor Create(aSizeCtrl: TSizeCtrl; aTarget: TControl); {$IFNDEF VER100} reintroduce; {$ENDIF}
    destructor Destroy; override;
  end;
  ...
  public
    ...
    procedure SizeCtrlMouseDown(Sender: TObject; Target: TControl; TargetPt: TPoint; var handled: boolean);
    procedure SizeCtrlSetCursor(Sender: TObject; Target: TControl; TargetPt: TPoint; var handled: boolean);
    procedure SizeCtrlKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
...
procedure Register;
begin
  RegisterComponents('Comps MIGUEL', [TSizeCtrl]);
end;
...
begin
  if not (fPos in fTargetObj.fSizeCtrl.fValidBtns)
  or (fTargetObj.fSizeCtrl.fMoveOnly) then begin
    Cursor := crDefault;
...
begin
  ...
  for i := low(TBtnPos) to high(TBtnPos) do begin
...
begin
  ...
  fMultiResize := true;
  fValidBtns := [bpLeft, bpTopLeft, bpTop, bpTopRight, bpRight, bpBottomRight,
...
{$ENDIF}
  ...
  OnDuringSizeMove := SizeCtrlDuring;
  OnEndSizeMove := SizeCtrlEnd;
  GridSize := GRIDSIZE;
  //PopupMenu := PopupMenu2;
  //to override behaviour of Pagecontrols so new pages can be selected ...
  OnMouseDown := SizeCtrlMouseDown;
  OnSetCursor := SizeCtrlSetCursor;
  OnKeyDown := SizeCtrlKeyDown;
...
var
  ...
  begin
    ...
    if assigned(fOnMouseDown) then begin
      ...
      if assigned(regCtrl) then begin
        handled := false;
...
begin
  case Msg.Msg of
    ...
    WM_PARENTNOTIFY:
      if not (TWMParentNotify(Msg).Event in [WM_CREATE, WM_DESTROY]) then begin
    ...
    WM_SETCURSOR:
      if (HIWORD(Msg.lParam) <> 0) then begin
        Msg.Result := 1;
        getCursorPos(screenPt);
        regCtrl := RegisteredCtrlFromPt(screenPt);

        handled := false;
        if assigned(fOnSetCursor) and assigned(regCtrl) then begin
          controlPt := regCtrl.ScreenToClient(screenPt);
          fOnSetCursor(self, RegisteredCtrlFromPt(screenPt), controlPt, handled);
        end;

        if handled then //do nothing
        else if TargetIndex(regCtrl) >= 0 then begin
    ...
    WM_KEYDOWN:
      begin
        ...
        case Msg.WParam of
          VK_UP:
            if ShiftKeyIsPressed then begin
              SizeTargets(0,-1);
          ...
          VK_ESCAPE:
            ...
            if fState <> scsReady then begin
              ...
              ClearTargets;
              if i >= 0 then AddTarget(TRegisteredObj(fRegList[i]).fControl);
            end;
        end;
      end;
...
begin
  ...
  if assigned(Control) then
    for i := 0 to fTargetList.Count -1 do
      if TTargetObj(fTargetList[i]).fTarget = Control then begin
        result := i;
        break;
      end;
end;
...
var TargetObj: TTargetObj;
begin
  result := -1;

  if (csDestroying in ComponentState)
  or (fState <> scsReady) then exit;

  result := TargetIndex(Control);

  if not assigned(Control)
  or not Control.Visible
  or (Control is TCustomForm)
  or (result >= 0) then exit;
...
var RegisteredObj: TRegisteredObj;
begin
  if not IsVisible(Control) then begin
    result := -1;
    exit;
  end;
...
begin
  ...
  for i := 0 to fRegList.Count -1 do
    if TRegisteredObj(fRegList[i]).fControl = Control then begin
      result := i;
      break;
    end;
end;
...
begin
  ...
  for i := fTargetList.Count -1 downto 0 do
    with TTargetObj(fTargetList[i]) do begin
      if not PointIsInControl(screenPt, fTarget) then continue;
      if not (fTarget is TWinControl) then begin
        result := fTarget;
        exit; //ie assume this is top-most since it can't be a parent.
      end else if not assigned(result) then
        result := fTarget
      else begin
...
begin
  ...
  for i := fRegList.Count -1 downto 0 do
    with TRegisteredObj(fRegList[i]) do begin
      if not PointIsInControl(screenPt, fControl) then continue;
      if not (fControl is TWinControl) then begin
        result := fControl;
        exit; //ie assume this is top-most since it can't be a parent.
      end else if not assigned(result) then
        result := fControl
...
begin
  ...
  if (Sender is TSizeBtn) then begin
  ...
  if fState = scsMoving then begin
    fClipRec := parentClientRec;
  end else
    with targetObj do begin //ie sizing
      fClipRec := fFocusRect;
...
begin
  ...
  if (fState = scsReady)
  or not assigned(fCapturedCtrl) then exit;
  ...
  if (fState = scsSizing) then begin
...
procedure TSizeCtrl.DoMouseUp(Sender: TObject; Button: TMouseButton; Shift: TShiftState);
var i: integer;
begin
...
var
  ...
  j: TBtnPos;
begin
  if fMultiResize or (TargetCount = 1) then begin
...
procedure TSizeCtrl.SizeCtrlTargetChange(Sender: TObject);
begin
  (*
  if TargetCount = 0 then
    StatusBar1.SimpleText := ''
  else with SizeCtrl.Targets[0] do
    StatusBar1.SimpleText :=
...
procedure TSizeCtrl.SizeCtrlDuring(Sender: TObject; dx,dy: integer; State: TSCState);
begin
  (*
  with Targets[0] do
...
procedure TSizeCtrl.SizeCtrlEnd(Sender: TObject; State: TSCState);
begin
  (*
...
procedure TSizeCtrl.SizeCtrlMouseDown(Sender: TObject; Target: TControl; TargetPt: TPoint; var handled: boolean);
var i: integer;
begin
  ...
  if (Target is TPageControl) then
    with TPageControl(Target) do begin
...
procedure TSizeCtrl.SizeCtrlSetCursor(Sender: TObject; Target: TControl; TargetPt: TPoint; var handled: boolean);
var i: integer;
begin
...
procedure TSizeCtrl.SizeCtrlKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
...
begin
  for i := 0 to aParent.ControlCount -1 do begin
    ...
    if (aParent.Controls[i].Tag = 0) then begin
